...
TEST(SmfSaveLoadTest, cMajorScale) {
    ...
    const std::vector<bw_music::Pitch> pitches{60, 62, 64, 65, 67, 69, 71, 72};
    {
        babelwires::ValueTreeRoot smfFeature(
            testEnvironment.m_projectContext.getService<babelwires::TypeSystem>(),
            babelwires::FileTypeT<smf::SmfSequence>::getType(testEnvironment.m_projectContext.getService<babelwires::TypeSystem>()));
...
TEST(SmfSaveLoadTest, format0Chords) {
    ...
    {
        babelwires::ValueTreeRoot smfFeature(testEnvironment.m_projectContext.getService<babelwires::TypeSystem>(),
                                             babelwires::FileTypeT<smf::SmfSequence>::getType(
                                                 testEnvironment.m_projectContext.getService<babelwires::TypeSystem>()));
        smfFeature.setToDefault();
