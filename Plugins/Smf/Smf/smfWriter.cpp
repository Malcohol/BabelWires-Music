...
smf::SmfWriter::WriteTrackEventResult smf::SmfWriter::writeTrackEvent(int channelNumber,
    ...
    if (channelNumber == -1) {
        ...
        if (const auto* pan = e.tryAs<bw_music::PanTrackEvent>()) {
            ...
            m_os->put(pan->getMinCentredMaxValue().getUnsigned<7>());
            return WriteTrackEventResult::Written;
        }
        ...
        if (const auto* pitchBend = e.tryAs<bw_music::PitchBendTrackEvent>()) {
            writeModelDuration(timeSinceLastEvent);
            const std::uint16_t encodedPitchBend = static_cast<std::uint16_t>(pitchBend->getMinCentredMaxValue().getUnsigned<14>());
