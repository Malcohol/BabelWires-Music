    ...
    class TrackEventConsumer {
      public:
        ...
        enum class EventHandlingResult {
            /// The event was ignored by the parser. It's time should be accumulated.
            Ignored,
            /// The event was handled by the parser.
            Handled,
            /// The parser has seen everything it needs to and should not receive any further events.
            Done
        };

        struct TimeInfo {
            /// MIDI ticks since the last event that was handled by the parser.
        ...
        virtual babelwires::ResultT<EventHandlingResult> onControlChange(TimeInfo timeInfo, std::uint8_t channel4,
        ...
        virtual babelwires::ResultT<EventHandlingResult> onTimeSignature(TimeInfo timeInfo,
            std::int8_t numerator, std::int8_t denominatorPower,
            std::int8_t midiClocksPerMetronomeClick, std::int8_t num32ndNotesPerMidiQuarterNote) {
            return EventHandlingResult::Ignored;
        }

        enum class KeySignatureMode { Major, Minor };

        virtual babelwires::ResultT<EventHandlingResult> onKeySignature(TimeInfo timeInfo, std::uint8_t m_numSharpsOrFlats,
                                                                       KeySignatureMode mode) {
            return EventHandlingResult::Ignored;
        }
