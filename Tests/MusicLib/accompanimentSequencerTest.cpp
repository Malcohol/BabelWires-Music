...
TEST_F(AccompanimentSequencerTest, BasicProcessing) {
    // Set a chord track
    bw_music::Track chordTrack = testUtils::getTrackOfChords({
        {bw_music::PitchClass::Value::C, bw_music::ChordType::Value::M, bw_music::ModelDuration(4)},
        {bw_music::PitchClass::Value::F, bw_music::ChordType::Value::m, bw_music::ModelDuration(4)},
        {bw_music::PitchClass::Value::G, bw_music::ChordType::Value::dim, bw_music::ModelDuration(4)}
    });
    ...
    // Get the result tracks
    {
        ...
        EXPECT_EQ(resultTrack1.getNumEvents(), bw_music_testplugin::SimpleAccompaniment::getNumEventsInTrack() * 4 * 3);
        EXPECT_EQ(resultTrack1.getDuration(), chordTrack.getDuration());
    }
    {
        bw_music::Track resultTrack2 = getResultTrack(1);
        EXPECT_EQ(resultTrack2.getNumEvents(), bw_music_testplugin::SimpleAccompaniment::getNumEventsInTrack() * 4 * 3);
