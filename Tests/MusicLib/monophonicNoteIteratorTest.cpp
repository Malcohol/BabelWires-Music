#include <gtest/gtest.h>

#include <MusicLib/Types/Track/trackBuilder.hpp>
#include <MusicLib/Utilities/monophonicNoteIterator.hpp>

#include <Tests/TestUtils/testTrackEvents.hpp>

#include <Tests/TestUtils/testLog.hpp>

namespace {
    bw_music::Track createTestTrack() {
        bw_music::TrackBuilder track;

        // Expect this note.
        bw_music::NoteOnEvent noteOn(0, 40);
        track.addEvent(noteOn);
        bw_music::NoteOffEvent noteOff(1, 40);
        track.addEvent(noteOff);
        // End of note

        // Expect this note.
        track.addEvent(noteOn);

        track.addEvent(testUtils::TestTrackEvent(1));

        noteOff.setTimeSinceLastEvent(1);
        noteOff.setPitch(40);
        track.addEvent(noteOff);
        // End of note

        track.addEvent(testUtils::TestTrackEvent(5));

        // Expect this note.
        noteOn.setTimeSinceLastEvent(0);
        noteOn.setPitch(50);
        track.addEvent(noteOn);

        track.addEvent(testUtils::TestTrackEvent(1));

        noteOn.setTimeSinceLastEvent(1);
        noteOn.setPitch(40);
        track.addEvent(noteOn);

        track.addEvent(testUtils::TestTrackEvent(1));
        track.addEvent(testUtils::TestTrackEvent(1));

        noteOff.setTimeSinceLastEvent(0);
        noteOff.setPitch(40);
        track.addEvent(noteOff);

        track.addEvent(testUtils::TestTrackEvent(1));

        noteOn.setTimeSinceLastEvent(1);
        noteOn.setPitch(40);
        track.addEvent(noteOn);

        noteOff.setTimeSinceLastEvent(1);
        noteOff.setPitch(50);
        track.addEvent(noteOff);
        // End of note

        track.addEvent(testUtils::TestTrackEvent(1));

        noteOff.setTimeSinceLastEvent(1);
        noteOff.setPitch(40);
        track.addEvent(noteOff);

        track.addEvent(testUtils::TestTrackEvent(1));

        return track.finishAndGetTrack();
    }
} // namespace

TEST(MonophonicNoteIterator, OnOffOnly) {
    testUtils::TestLog log;

    bw_music::Track track = createTestTrack();

    auto [it, end] = bw_music::iterateOverMonotonicNotes(track, bw_music::MonophonicNoteIterator::OnOffOnly);

    ASSERT_NE(it, end);
    ASSERT_NE(it->tryAs<bw_music::NoteOnEvent>(), nullptr);
    EXPECT_EQ(it->tryAs<bw_music::NoteOnEvent>()->getPitch(), 40);
    EXPECT_EQ(it->tryAs<bw_music::NoteOnEvent>()->getTimeSinceLastEvent(), 0);

    ++it;
    ASSERT_NE(it, end);
    ASSERT_NE(it->tryAs<bw_music::NoteOffEvent>(), nullptr);
    EXPECT_EQ(it->tryAs<bw_music::NoteOffEvent>()->getPitch(), 40);
    EXPECT_EQ(it->tryAs<bw_music::NoteOffEvent>()->getTimeSinceLastEvent(), 1);

    ++it;
    ASSERT_NE(it, end);
    ASSERT_NE(it->tryAs<bw_music::NoteOnEvent>(), nullptr);
    EXPECT_EQ(it->tryAs<bw_music::NoteOnEvent>()->getPitch(), 40);
    EXPECT_EQ(it->tryAs<bw_music::NoteOnEvent>()->getTimeSinceLastEvent(), 0);

    ++it;
    ASSERT_NE(it, end);
    ASSERT_NE(it->tryAs<bw_music::NoteOffEvent>(), nullptr);
    EXPECT_EQ(it->tryAs<bw_music::NoteOffEvent>()->getPitch(), 40);
    EXPECT_EQ(it->tryAs<bw_music::NoteOffEvent>()->getTimeSinceLastEvent(), 2);

    ++it;
    ASSERT_NE(it, end);
    ASSERT_NE(it->tryAs<bw_music::NoteOnEvent>(), nullptr);
    EXPECT_EQ(it->tryAs<bw_music::NoteOnEvent>()->getPitch(), 50);
    EXPECT_EQ(it->tryAs<bw_music::NoteOnEvent>()->getTimeSinceLastEvent(), 5);

    ++it;
    ASSERT_NE(it, end);
    ASSERT_NE(it->tryAs<bw_music::NoteOffEvent>(), nullptr);
    EXPECT_EQ(it->tryAs<bw_music::NoteOffEvent>()->getPitch(), 50);
    EXPECT_EQ(it->tryAs<bw_music::NoteOffEvent>()->getTimeSinceLastEvent(), 7);

    ++it;
    ASSERT_EQ(it, end);
}
