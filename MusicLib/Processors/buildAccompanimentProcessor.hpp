...
namespace bw_music {
    class MUSICLIB_API BuildAccompanimentProcessorInput : public babelwires::GenericType {
      public:
        DOWNCASTABLE(BuildAccompanimentProcessorInput, babelwires::GenericType);
        REGISTERED_TYPE("BuildAccompIn", "Build Accompaniment In", "d1f8b0c2-3f4e-4c5a-9b6e-7c8d9e0f1a2b", 1);

        BuildAccompanimentProcessorInput(const babelwires::TypeSystem& typeSystem);
    ...
    class MUSICLIB_API BuildAccompanimentProcessorOutput : public babelwires::GenericType {
      public:
        DOWNCASTABLE(BuildAccompanimentProcessorOutput, babelwires::GenericType);
        REGISTERED_TYPE("BuildAccompOut", "Build Accompaniment Out", "e3f4b5c6-7d8e-9f0a-b1c2-d3e4f5a6b7c8", 1);

        BuildAccompanimentProcessorOutput(const babelwires::TypeSystem& typeSystem);

        static babelwires::ShortId getIdOfResult();
    };

    /// Processor that builds a record of chord-adjusted tracks from an input type and ChordTypeSet.
    class MUSICLIB_API BuildAccompanimentProcessor : public babelwires::Processor {
      public:
