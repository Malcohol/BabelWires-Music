...
std::size_t bw_music::PressureEvent::getHash() const {
    return babelwires::hash::mixtureOf(static_cast<const char*>("ChannelPressure"), m_timeSinceLastEvent, m_pressure);
}
...
bool bw_music::PressureEvent::doIsEqualTo(const TrackEvent& other) const {
    ...
    return TrackEvent::doIsEqualTo(other) && (m_pressure == otherChannelPressure.m_pressure);
}
