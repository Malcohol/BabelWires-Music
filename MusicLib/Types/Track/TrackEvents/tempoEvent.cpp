...
std::size_t bw_music::TempoEvent::getHash() const {
    return babelwires::hash::mixtureOf(static_cast<const char*>("Tempo"), m_timeSinceLastEvent, m_tempo);
}
...
bool bw_music::TempoEvent::doIsEqualTo(const TrackEvent& other) const {
    ...
    return TrackEvent::doIsEqualTo(other) && (m_tempo == otherTempo.m_tempo);
}